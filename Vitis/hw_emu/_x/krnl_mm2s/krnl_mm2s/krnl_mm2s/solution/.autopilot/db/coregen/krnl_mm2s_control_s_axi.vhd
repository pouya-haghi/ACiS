...
entity krnl_mm2s_control_s_axi is
generic (
...
end entity krnl_mm2s_control_s_axi;
...
-- 0x20 : reserved
-- 0x24 : Data signal of dest
--        bit 31~0 - dest[31:0] (Read/Write)
-- 0x28 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of krnl_mm2s_control_s_axi is
    ...
    constant ADDR_DEST_DATA_0 : INTEGER := 16#24#;
    constant ADDR_DEST_CTRL   : INTEGER := 16#28#;
    ...
    signal int_dest            : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_DEST_DATA_0 =>
                        rdata_data <= RESIZE(int_dest(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    dest                 <= STD_LOGIC_VECTOR(int_dest);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_DEST_DATA_0) then
                    int_dest(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_dest(31 downto 0));
                end if;
            end if;
        end if;
    end process;
